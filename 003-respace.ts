...
function respaceInternal(spacelessMessage: string, words: string[], startIndex: number): string[][] {
  ...
  for (const word of words) {
    if (spacelessMessage.startsWith(word, startIndex)) {
      for (const subCombination of respaceInternal(spacelessMessage, words, startIndex + word.length)) {
