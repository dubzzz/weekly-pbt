...
function buildDirectedGraphFromTasks(tasks: Task[]): DirectedGraph {
  ...
  const edges = tasks
    .map(
      (task): InternalTask => ({
        ...
        dependsOnTasks: task.dependsOnTasks.map((taskId) => tasks.findIndex((t) => t.taskId === taskId)),
      })
    )
    .flatMap((task, index): DirectedEdge[] => {
      return [
        ...
        ...task.dependsOnTasks.map((dependsOnTaskId) => ({ from: dependsOnTaskId + n, to: index, weight: 0 })),
      ];
    });
  ...
  return {
    ...
    finishVertexFor: (taskId) => tasks.findIndex((t) => t.taskId === taskId) + n,
  };
}
...
function postDfsInternal(g: DirectedGraph, v: number, marked: Set<number>, acc: number[]): void {
  marked.add(v);
