...
/**
 * Given a map of all the stations and tracks connecting them,
 * we want to compute the shortest path (in terms of tracks to take) to reach the destination.
 *
 ...
 * @param stations - List containing all the known stations (stations outside this list will be ignored even if tracks leading to them exist)
...
export function metroRoute(
  ...
  const distanceToNode = new Map(stations.map((node) => [node, { distance: Number.POSITIVE_INFINITY, edges: [] }]));
